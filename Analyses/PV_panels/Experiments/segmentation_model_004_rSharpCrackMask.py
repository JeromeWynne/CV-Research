...
def variables():
    ...
    v = { 
          ...
          'N' : cnn_utils.BatchNormalizedConvolutionalLayer(map_size = 5, in_channels = N_CHANNELS, out_channels = N_CHANNELS - 1,
                                                            residual_layer = False),
        
          'H' : cnn_utils.BatchNormalizedConvolutionalLayer(map_size = 1, in_channels = N_CHANNELS - 1, out_channels = 2,
...
def graph(variables, images, label_masks, mode = 'training'):
    """
       ...
       images and labels are both tensors (images is a stack of images, labels is an image-shaped stack of one-hot-encoded labels).
    ...
    RECURRENCY_PERIOD = 3 # Increasing this will dramatically push up the training time...
    H = []
    for i in range(RECURRENCY_PERIOD):
        N = variables['N'].apply(input_data = tf.concat([images, N], axis = -1), mode = mode)
        H.append(variables['H'].apply(input_data = N, mode = mode)) # retain the output of each layer for our
                                                                    # loss function, which will penalize mistakes at each layer.
    logits = H[-1]
    cross_entropy, predictions, iou = cnn_utils.get_loss_predictions_iou(logits, label_masks)
    
    # We compute the cross-entropy for all layers
    reshaped_labels = tf.reshape(label_masks, shape = [-1, 2])
    recurrent_cross_entropy = 0
    for i in range(len(H) - 1):
                recurrent_cross_entropy += tf.nn.softmax_cross_entropy_with_logits(
                                                        logits = tf.reshape(H[i], shape = [-1, 2]),
                                                        labels = reshaped_labels)
    recurrent_cross_entropy += cross_entropy
    
    return recurrent_cross_entropy, predictions, iou