...
with simple_graph.as_default():
    ...
    with tf.name_scope('Variables'):
        with tf.name_scope('VariablesConvLayers'):
            filters1 = tf.Variable(tf.truncated_normal(
                    	shape = [filter_size[0], filter_size[0], n_channels, output_channels[0]], stddev = 0.01),
			name = 'Layer_1_Filters')
            biases1  = tf.Variable(tf.zeros([output_channels[0]]),
			name = 'Layer_1_Biases')
    
            filters2 = tf.Variable(tf.truncated_normal(
                        shape = [filter_size[1], filter_size[1], output_channels[0], output_channels[1]], stddev = 0.01),
        ...
        with tf.name_scope('VariablesFCLayer'):
            weights3 = tf.Variable(tf.truncated_normal(
                        shape = [output_channels[1] * image_size * image_size, n_classes], stddev = 0.01),
    ...
    with tf.name_scope('Training'):
        ...
        with tf.name_scope('Optimizer'):
            optimizer = tf.train.GradientDescentOptimizer(0.05, name = 'optimizer').minimize(loss)
    
        # Feedback information
        trp = tf.nn.softmax(logits)

    tf_train_predictions = tf.identity(trp, name = 'tf_train_predictions')

    with tf.name_scope('Testing'):
        tep = tf.nn.softmax(model(tf_test_data))

    tf_test_predictions  = tf.identity(tep, name = 'tf_test_predictions')
    merged = tf.summary.merge_all()
print('Graph defined.')

### > TRAINING <
spec = {'mode':'holdout', 'seed':3,
        'batch_size':batch_size,
        'training_steps':3001} # Testing spec.
