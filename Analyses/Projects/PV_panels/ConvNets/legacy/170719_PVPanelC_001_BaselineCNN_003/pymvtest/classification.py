...
class Tester(object):
    ...
    def evaluate_model(self):
        ...
        with tf.Session(graph = self.tf_graph) as session:
            ...
            if exists('./output/'): rmtree('./output') # Remove previous results

            session.run(tf.global_variables_initializer())
            train_writer  = tf.summary.FileWriter('output/train', session.graph)
            valid_writer  = tf.summary.FileWriter('output/valid', session.graph)
            test_writer   = tf.summary.FileWriter('output/test',  session.graph)
