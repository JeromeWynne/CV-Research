...
\begin{itemize}
	...
	\item Euclidean balls and ellipsoids (linear transformations of a unit ball). $\{x | (x - x_c)P^{-1}(x - x_c) \leq 1\} = \{x_c + Au | \|u\|_2 \leq 1\}$. A euclidean ball is the set of points that satisfy $\{x| \|x - x_c\|_2 \leq r\}$. We also defined a norm as a function that satisfies various conditions.
