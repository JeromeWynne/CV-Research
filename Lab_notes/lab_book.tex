...
\labday{Wednesday, 12th July 2017}
...
\labday{Thursday, 13th July 2017}
...
\labday{Saturday, 15th July 2017}
...
\experiment{Prince, \emph{Computer Vision} (Ch. 10: Graphical Models)}
...
\experiment{Prince, \emph{Computer Vision} (Ch. 11: Models for Chains and Trees)}
\begin{itemize}
...
\labday{Wednesday, 19th July 2017}
...
\labday{Thursday, 20th July 2017}
...
\labday{Saturday, 22nd July 2017}
...
\tikzstyle{node}=[circle,
	...
	\node (u_1) [node] {$u^{(1)}$}; &\node (u_5) [node] {$u^{(5)}$};  & & \node (inv1) {}; & & \\
	\node (u_2) [node] {$u^{(2)}$}; & \node (u_6) [node] {$u^{(6)}$}; & & \node (inv2) {}; & \node (u_n-2) [node] {$u^{(n-2)}$}; & \node (u_n) [node] {$u^{(n)}$}; \\
	\node (u_3) [node] {$u^{(3)}$};	 & \node (u_7) [node] {$u^{(7)}$}; & & \node (inv3) {}; & \node (u_n-1) [node] {$u^{(n-1)}$}; & \\
	\node (u_4) [node] {$u^{(4)}$};	 & \node (u_8) [node] {$u^{(8)}$}; & & \node (inv4) {}; &  & \\
	};
	
	\path[->] % Specify path type as an option
	% First layer's connections
		(u_1) edge (u_5)
		(u_1) edge (u_6)
		(u_1) edge (u_7)
		(u_1) edge (u_8)
	...
	% Intermediate layer's connections
		(u_5) -- node[auto=false]{\ldots} (inv1)
		(u_6) -- node[auto=false]{\ldots} (inv2)
		(u_7) -- node[auto=false]{\ldots} (inv3)
		(u_8) -- node[auto=false]{\ldots} (inv4)
		
	% Penultimate layer's connections
		(inv1) edge (u_n-1)
		(inv2) edge (u_n-1)
		(inv3) edge (u_n-1)
		(inv4) edge (u_n-1)
	...
	% Matrix
	\matrix[row sep=0.5cm, column sep = 1cm]{
	\node (u_1) [node] {$\frac{\partial u^{(n)}}{\partial u^{(1)}}$};	 &\node (u_5) [node] {$\frac{\partial u^{(n)}}{\partial u^{(5)}}$};  & & \node (inv1) {}; & & \\
	\node (u_2) [node] {$\frac{\partial u^{(n)}}{\partial u^{(2)}}$}; 	& \node (u_6) [node] {$\frac{\partial u^{(n)}}{\partial u^{(6)}}$}; & & \node (inv2) {}; & \node (u_n-2) [node] {$\frac{\partial u^{(n)}}{\partial u^{(n-2)}}$}; & \node (u_n) [node] {$u^{(n)}$}; \\
	\node (u_3) [node] {$\frac{\partial u^{(n)}}{\partial u^{(3)}}$};	 & \node (u_7) [node] {$\frac{\partial u^{(n)}}{\partial u^{(7)}}$}; & & \node (inv3) {}; & \node (u_n-1) [node] {$\frac{\partial u^{(n)}}{\partial u^{(n-1)}}$}; & \\
	\node (u_4) [node] {$\frac{\partial u^{(n)}}{\partial u^{(4)}}$};	 & \node (u_8) [node] {$\frac{\partial u^{(n)}}{\partial u^{(8)}}$}; & & \node (inv4) {}; &  & \\
	};
...
\caption{\emph{(Top)} A multi-layer perceptron. \emph{(Bottom)} Its associated backpropagation graph.}
\end{figure}


\labday{Sunday, 23rd July 2017}
(13:30 - 15:00) Learnt to use Tikz, described back-propagation work from yesterday. One important point I neglected to mention was that the derivative of the network's output w.r.t. an arbitrary tensor can be understood using the example same principle.
(15:00 - 17:30) Reading Goodfellow's \emph{Deep Learning} - Hessian, ill-conditioning, understanding the behaviour of the cost surface in high-dimensional feature spaces, understanding why it is necessary to shrink the learning rate as we approach convergence (magnitude of the Hessian's eigenvalues increases).

(17:30 - 21:30) Adding HOGs to model.


\labday{3rd August 2017}

\experiment{Serial Peripheral Interface}
